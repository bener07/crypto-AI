...
class Mokita:
    ...
    def visualize_training_results(results):
        ...
        def validater(self, n_per_in, n_per_out):
            ...
            for i in range(n_per_in, len(df)-n_per_in, n_per_out):
                ...
                yhat = model.predict(np.array(x).reshape(1, n_per_in, self.n_features))
    ...
    def predict(self):
        if self.neuralnet is None:
            self.createNn()
        df = pd.read_csv(f'../../api/app/csvs/{self.coin}/info.csv')
        ## Datetime conversion
        ...
        yhat = model.predict(np.array(df.tail(self.n_per_in)).reshape(1, self.n_per_in, self.n_features))
        ...
        actual = pd.DataFrame(self.close_scaler.inverse_transform(df[["Close"]].tail(pers)),
    ...
    def createNn(self):
        ...
        X, y = self.split_sequence(df.to_numpy(), n_per_in, n_per_out)
